...
<table  width="100%">
	<div>
		<thead> 
	<th style ="width: 60%;font-size: 16px;text-align: left;">Quote No: @php echo rand() @endphp <br>Quote Date: @php echo date("d/m/Y"); @endphp <br> Due Date:@php echo date('d/m/Y', strtotime('+1 months'));  @endphp</th>
...
<table style="width: 100%">
		...
		<thead> 
	@if($organ_get)
	<th style ="width: 60%;font-size: 16px;text-align: left;padding-left: 30px;">Company Name:	{{$organ_get->orgName}} <br>Address:		{{$organ_get->orgAddress}} <br>Country:	{{$organ_get->country}} <br>GSTIN ID: <br>VAT No:</th>
	</thead>
	@endif
